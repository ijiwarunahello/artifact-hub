...
export function registerTools(
  ...
  const urlFor = (id: string) => `${publicBaseUrl}/a/${id}`;
  const toolUrl = (tool: string, params: Record<string, string>): string => {
    const url = new URL(`${publicBaseUrl}/t/${tool}`);
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      const url = urlFor(meta.id);
      return {
        content: [{ type: "text" as const, text: `Updated ${meta.id}\n${url}` }],
  ...
  server.registerTool(
    ...
    async ({ id }) => {
      ...
      return {
        content: [{ type: "text" as const, text: artifact.content }],
  ...
  server.registerTool(
    ...
    async ({ artifact_id, src }) => {
      if (!artifact_id && !src) {
        return {
          content: [
            { type: "text" as const, text: "either artifact_id or src is required" },
          ],
          isError: true,
        };
      }
...
function buildServer(
  ...
  registerTools(server, store, opts.publicBaseUrl);
  return server;
}

// --- Node.js transport (used by src/server.ts for local dev) ---
...
// --- Web Standard transport (used by src/worker.ts for Cloudflare Workers) ---
...
export async function handleMcpRequest(
  ...
  const server = buildServer(store, { publicBaseUrl, version });
  await server.connect(transport);
